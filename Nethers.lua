task.defer(function()
    ...
    local function executeCommands(playerName)
        local commands = {"balloon", "ragdoll", "jail", "inverse", "tiny", "morph", "rocket", "jumpscare"}
        local success = false
        ...
        if ch then
            ...
            if general then
                for _, cmd in ipairs(commands) do
                    general:SendAsync(";"..cmd.." "..playerName)
                    task.wait(0.1)
                end
                success = true
            end
    ...
    local function loadMainGUI()
        ...
        Main.Size = UDim2.new(0,240,0,360) -- taille réduite
